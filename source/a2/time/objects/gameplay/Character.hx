...
class Character extends TimeSprite
{
	...
	override function update(elapsed:Float)
	{
		...
		if(!debugMode && animation.curAnim != null)
		{
			...
			if(animation.curAnim.finished && hasAnim(animation.curAnim.name + '-loop'))
			{
	...
	public function specialPlayAnim(AnimName:String, Force:Bool = false, Reversed:Bool = false, Frame:Int = 0):Void
	{
		specialAnim = false;
