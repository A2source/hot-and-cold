...
If you have any questions about the editor, ask me!';
	...
	override function create()
	{
		...
		timeText = new FlxText(TEXT_X, FlxG.height, 0, '', 16);
		timeText.x -= timeText.height + 5;
		timeText.scrollFactor.set();
		add(timeText);

		infoText = new FlxText(TEXT_X, timeText.y - SPACE_BETWEEN_TEXT, 0, '', 16);
		infoText.scrollFactor.set();
		add(infoText);

		infoText.origin.set(0, infoText.height / 2);
		...
		zoomTxt = new FlxText(Main.fpsVar.width + 5, 10, 0, "Zoom: 1 / 1", 16);
		zoomTxt.borderStyle = FlxTextBorderStyle.OUTLINE;
		zoomTxt.borderColor = 0xFF000000;
		zoomTxt.borderSize = 3;
		...
		charIndicator.setPosition(FlxG.width - charIndicator.width, FlxG.height - charIndicator.height);
		charIndicator.scrollFactor.set();
		add(charIndicator);

		indicator_isPlayer = new FlxSprite(charIndicator.x - 65, charIndicator.y);
	...
	{
		var formatBox = new haxe.ui.containers.HBox();

		UiS.addSpacer(525, 0, formatBox);
		...
		formatBox.addComponent(charLeft);
		...
		formatBox.addComponent(charRight);
	...
	override function update(dt:Float)
	{
		...
		zoomTxt.x = Main.fpsVar.width + 12;
	...
	function updateUIText(dt:Float)
	{
		...
		infoText.text = '$curSec, $curBeat, $curStep, ${quantization}th, ${FlxStringUtil.formatTime(secondsTotal, false)}';
	}
	...
	function updateHeads():Void
	{
		...
		indicator_isPlayer.x = charIndicator.x - 65;
	}
